...
const ShoppingCart = {
    ...
    isLocalStorageAvailable: function() {
        try {
            ...
            return false; // localStorage unavailable (private mode or cookies disabled)
        }
    },
    ...
    addItem: function(product) {
        ...
        if (existingIndex > -1) {
            // Increase quantity
            var newQty = (cart[existingIndex].quantity || 1) + 1;
            cart[existingIndex].quantity = Math.min(newQty, 99); // cap at 99
    ...
    updateQuantity: function(productId, quantity) {
        ...
        if (index > -1) {
            if (quantity <= 0) {
                ...
                if (isNaN(q) || q < 1) { this.log('WARNING: invalid quantity, defaulting to 1'); q = 1; }
    ...
    showNotification: function(message, type = 'info') {
        ...
        // Build notification via DOM (not innerHTML) to prevent XSS from raw message
        const inner = document.createElement('div');
        const bg = type === 'success' ? '#4CAF50' : type === 'error' ? '#f44336' : '#2196F3';
        inner.style.cssText = 'position:fixed;top:20px;right:20px;background:' + bg
            + ';color:white;padding:15px 25px;border-radius:5px;box-shadow:0 4px 12px rgba(0,0,0,.3)'
            + ';z-index:10000;font-family:Lato,sans-serif;font-size:14px;font-weight:500;animation:slideInRight .3s ease';
        inner.textContent = message; // textContent — never executes HTML
        notification.appendChild(inner);
        
        // Inject keyframe styles only once
        if (!document.getElementById('sj-cart-anim-style')) {
            const style = document.createElement('style');
            style.id = 'sj-cart-anim-style';
            style.textContent = `
    ...
    init: function() {
        ...
        if (this.DEBUG) console.log('✅ ShoppingCart initialized successfully');
    }
};
